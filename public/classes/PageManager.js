export class PageManager {
    ...
    get_range(text, page, n) {
        let start = this.get_actual_start(text, page);
        if (page + n >= this.positions.length)
            return { start: start, end: undefined };
        return {
            start: start,
            end: this.positions[page + n] + 1
        };
    }
    on_insert(text, start, end) {
        const res = { page: this.page_at_index(start), updated: 1, delta: 0 };
    ...
    page_at_index(index) {
        ...
        for (let i = 0; i < this.positions.length - 1; i++) {
            if (this.positions[i] < index && index <= this.positions[i + 1])
                return i;
        }
    ...
    search_for(text, start, end, word) {
        ...
        while (c < end) {
            let i = 0;
            while (c < end && text[c] != word[i])
                c++;
            while (c + i < end && i < word.length && text[c + i] == word[i])
                i++;
            if (i == word.length)
                return c;
            c++;
    ...
    check_around_cursor(text, cursor, tags, max_distance = 20) {
        ...
        while (cursor + max_distance - start > 9) {
            ...
            while (end - cursor < max_distance && text[end] != ']')
                end++;
            if (text[end] != ']')
                return -1;
    ...
    detect_removed(text, start, end) {
        ...
        for (let i = 1; i < this.positions.length; i++) {
            if (this.positions[i] >= end + this.max_distance)
                break;
            if (this.is_affected(text, i, start, end))
                pages.push(i);
        }
        return pages;
    }
